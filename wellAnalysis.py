...
from tools import (
    load_merge_header_and_production_csv,
    swap_production_dates_for_time_delta,
    current_selection,
    decline_curve,
    handle_numerical_variables,
    handle_dateTime_variables,
    handle_object_variables,
    plot_map,
    fit_decline_curve,
    add_BOE_per_day_column,
    nominal_decline,
)
...
def main(headerCSV, productionCSV):
    ...
    print('\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
    wellByName = input('would you like to select individual wells by API-UWI number? [y/n]: ')
    # check user input
    while wellByName not in ('y', 'n', 'Y', 'N'):
        wellByName = input('please try again [y/n]?  ')
    ...
    print('\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
    wellByName = input('would you like to select wells near a GPS location? [y/n]: ')
    # check user input
    while wellByName not in ('y', 'n', 'Y', 'N'):
        wellByName = input('please try again [y/n]?  ')
    ...
    print('\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
    wellByVariable = input('would you like to subset wells by column values? [y/n]: ')
    # check user input
    while wellByVariable not in ('y', 'n', 'Y', 'N'):
        wellByVariable = input('please try again [y/n]?  ')
    ...
    print('\n~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~\n')
    b_value = None
    # determine if the user wants to pre-specify any of the decline curve parameters
    fixed_b = input('would you like to pre-specify the decline curve b-factor? [y/n]: ')
    # check user input
    while fixed_b not in ('y', 'n', 'Y', 'N'):
        fixed_b = input('please try again [y/n]?  ')

    if fixed_b.upper() == 'Y':
        while True:
            try:
                b_value = float(input('Enter value for b-factor: '))
            except ValueError:
...
class Quick_TypeCurve_Analysis(object):
    ...
    def subset_wells_by_distance(self):
        # obtain longitude and latitudes from the user
        while len(self.userLocation) != 2:
            while True:
                try:
                    self.userLocation = input(
        ...
        # obtain the selection radius from the user
        while True:
            try:
        ...
        # add vicinity column to data set
        ...
        for i, (lat, lon) in enumerate(zip(self.wellDF['Surface Latitude (WGS84)'], self.wellDF['Surface Longitude (WGS84)'])):
            dist[i] = geodesic([lat, lon], self.userLocation).miles

        self.wellDF['vicinity'] = dist

        # keep only wells within the user-selected radius
        self.wellDF = self.wellDF.loc[self.wellDF['vicinity'] <= userRadius]

        # notify the user of changes to the current selection
    ...
    def subset_by_well_name(self):
        ...
        print('\nSelect one or more of the following wells by API/UWI number\n')
        print('all wells available...')
        for i, well in enumerate(allWells):
            print('%i -- %s' % (i, well))

        selection = input('well selection [separate by commas]:\n')
    ...
    def subset_well_by_variable(self):
        ...
        print('\nSelect one or more of the following variables\n')
        print('all variables available...')

        # generate a dictionary of variables
        variableDict = dict()
        ...
        while len(selectedVars) == 0:
            try:
                selection = input('Select the variables by their number [separate multiple selections by commas]:\n')
                selectionList = [x.strip() for x in selection.split(',')]
        ...
        for colName in selectedVars:
            ...
            elif str(self.wellDF[colName].dtypes) in ['datetime64', 'timedelta64[ns]', 'datetime64[ns]']:
    ...
    def generate_type_curve(self, b_value=None):
        # get time delta column from selected wells
        self.wellDF = swap_production_dates_for_time_delta(self.wellDF)

        # decline curve estimated parameters
    ...
    def map_selected_wells(self):
        ...
        # send data to the mapping function
    ...
    def plot_individual_wells_and_type_curves(self):
        ...
        for well in np.unique(self.wellDF['API/UWI']):
            ...
            # add data to the list for saving to excel
